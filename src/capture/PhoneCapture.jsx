...
export default function PhoneCapture({ initial }) {
  ...
  const body=useMemo(()=>{
    ...
    return <div className="cs-mobile-done"><span>✓</span><h1>सब सही है?</h1><dl><dt>सामान</dt><dd>{draft.data.name||'यह सामान'}</dd><dt>श्रेणी</dt><dd>{categoryChoices.find(c=>c[0]===draft.data.category)?.[2]||draft.data.category}</dd><dt>मटेरियल</dt><dd>{materialChoices.find(c=>c[0]===draft.data.material)?.[2]||draft.data.material}</dd><dt>गिनती</dt><dd>{countTotal(draft.data)}</dd><dt>जगह</dt><dd>{draft.data.locations?.[0]?.name||'—'}</dd><dt>नाप</dt><dd>{draft.data.length} × {draft.data.width} × {draft.data.height} cm</dd><dt>वज़न</dt><dd>{draft.data.weight_g} g</dd></dl><button className="cs-primary cs-mobile-action" onClick={()=>updateData(d=>({...d,reviewStatus:'submitted',submittedAt:new Date().toISOString()}),'submitted')}>✓ जाँच के लिए भेजें</button><button onClick={()=>{setValue(null);setStage('category');}}>जानकारी सुधारें</button></div>;
  ...
  return <section className="cs-phone-panel">
    ...
    {camera==='product'&&<CameraCapture key={`${initial.id}-${shot}`} autoSave productName={draft.data.name} shotName={currentShot.hindi} guidance={currentShot.direction} icon={currentShot.icon} onUse={savePhoto} onClose={()=>setCamera('')}/>}
    {camera==='location'&&<CameraCapture key={`${initial.id}-location`} autoSave productName={draft.data.name} shotName="रैक की फोटो" guidance="रैक का नंबर और सामान दोनों दिखाएँ।" icon="🗄️" onUse={saveLocationPhoto} onClose={()=>setCamera('')}/>}
  </section>;
}
